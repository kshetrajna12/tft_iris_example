...
def _parse_function(proto, fs):
    ...
    return [parsed_features['petal_length_normalized'],
...
def load_dataset(input_path, tft_output_dir, 
                 batch_size=10, shuffle_buffer=2):
    _parse_fn = get_parse_function(tft_output_dir=tft_output_dir)
    filenames = [os.path.join(input_path, x) for x in tf.io.gfile.listdir(input_path)]
    dataset = tf.data.TFRecordDataset(filenames)
    dataset = dataset.map(_parse_fn)
    dataset = dataset.batch(batch_size)
    return dataset

dataset = load_dataset(input_path=TFRECORD_PATH,
                      tft_output_dir=TFT_OUTPUT_DIRECTORY)
...
signature_dict = { 
    'petal_width': [tf.TensorSpec(shape=[], dtype=tf.float32, name="petal_width")],
    "petal_length": [tf.TensorSpec(shape=[], dtype=tf.float32, name="petal_length")],
    'sepal_width': [tf.TensorSpec(shape=[], dtype=tf.float32, name="sepal_width")],
    "sepal_length": [tf.TensorSpec(shape=[], dtype=tf.float32, name="sepal_length")],
} 
...
class MyModel(tf.keras.Model):
    ...
    def __init__(self, model, tft_output_dir):
        ...
        self.model = model
        self.tft_layer = tft.TFTransformOutput(tft_output_dir).transform_features_layer()
    
    def call(self, inputs):
        transformed =  self.tft_layer(inputs)

        modelInput = tf.stack([transformed['petal_length_normalized'],
                    transformed['petal_width_normalized'],
                    transformed['sepal_length_normalized'],
                    transformed['sepal_width_normalized']], axis=1)
